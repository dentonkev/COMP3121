...
- number of augmenting paths can be up to vale of **max flow**, denoted $|f|$.
- augmenting path found in $O (V + E)$ (DFS, BFS). In a sensible flow network $V ≤ E + 1$, so we can write this as simply $O(E)$.
...
- number of augmenting paths is $O(VE)$, and since each takes $O(E)$ to find, the time complexity is $O(VE^2)$.
- $O(E|f|)$ still applies (from Ford-Fulkerson)