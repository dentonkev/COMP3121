...
<img src="./images/gale.png" width='400px' height='auto'>  
...
<img src="./images/m10.png" width='400px' height='auto'>  
...
The fast Fourier transform (FFT) is an algorithm that computes the Discrete Fourier Transform (DFT)

**High Level Explanation**  
Example: $S \times V$:
- Use $FFT(S) \rightarrow DFT(S)$
- Use $FFT(V) \rightarrow DFT(V)$
...
### Size Heuristic
The size heuristic guarantees that each tree in the data structure has height at most $log_2n$.
...
For each edge $e = (u, v)$ in the **sorted list of edges** use two `FIND` operations
  - if `FIND(u) = FIND(v)` continue
  - if `FIND(u) != FIND(v)` then perform `UNION(u, v)`, placing an edge and merging the tree

Analysing time complexity, overall --> $O(mlogn)$, where $m$ = edges and $n$ = verticies 
  
<img src="./images/t.png" width='450px' height='auto'>  
...
- direct graph must be acyclic to be topologically ordered
 ...
 **Problem**: Determine whether a string $B = b_1b_2 . . . b_m$ (the pattern) appears as a (contiguous) substring of a much longer string $A = a_1a_2 . . . a_n$ (the text).
 ...
 - Naive algorithm would run in $O(mn)$

### Rabin-Karp Algorithm
<img src="./images/rk1.png" width='400px' height='auto'>  
...
<img src="./images/rk.png" width='450px' height='auto'>  
<img src="./images/rk2.png" width='450px' height='auto'>  

**Analysis**
- Average case: $O(n - m + 1)$, $n$ text, $m$ pattern
  - Iterating through the text stops at the $(n - m + 1)$th character
- Worst case: $O(mn)$
...
<img src="./images/k.png" width='450px' height='auto'>  
<img src="./images/k1.png" width='450px' height='auto'>  
...
<img src="./images/kmpp.png" width='450px' height='auto'>  


- Compute the failure function ($\pi$) for the **pattern**
  - have a single pointer $j = 0$ in the table
- Have single pointer $i = 1$ in the longer string $A$ and compare characters at $A[i]$ and $\pi[j + 1]$
  - if $A[i]$ = $\pi[j + 1]$, $i = i + 1$ and $j = j + 1$
  - if $A[i]$ != $\pi[j + 1]$, then 
    - if $j = 0$, then $i = i + 1$
    - otherwise $j = \pi[j]$

**Analysis**
- $O(m + n)$, for iterating through pattern and text
...
- $V$ is **NP-hard** if it is at least as hard as everything in class **NP**
